...
**v0.19.10**。Claude Code、Codex、Cursorのnative integrationとGrokを含むMCP登録を持つ。runtime error収集は
...
- **実行中発火（PostToolUse / PostToolUseFailure hook、v0.10 / v0.14）— 非同期パイプライン**:
  ...
  - 非同期 worker (`caveat hook worker <workFile>`): work file を読んで unlink → `findCaveatsForHook(db, { topicText, failureText, surface: 'tool_error' })` を走らせる。主題anchorは両field、症状根拠は`failureText`だけから採る。hit > 0 なら`toolErrorReminderText(hits)`をpendingへ積む
...
- Claude commands、hooks、MCP 登録、markdown entry format、transcript の前提は、Claude Code の既存契約として維持する。
- Codex・Cursor 対応は adapter、policy helper、fixture、別文書で足し、Claude-facing の field 名や hook 出力文を他 host の都合で変えない。
- 他 host の方針と実行メモは `docs/03_dual_agent_support.md` を正とする。
