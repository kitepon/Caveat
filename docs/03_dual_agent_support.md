...
関連度0.8以上の知見だけ原文の対処を短く通知する。候補が無い場合は一度だけ検索を促す。
同じ完了ターンは再判定せず、通知済みの知見IDは同一セッションで再通知しない。
完了した判定は通知されなかった案件も`<caveatHome>/jev-observations/`へ保存する。
点数、候補語の確率、AND検索の結果、Jevの候補点数、実際の通知文と配送状態を残し、
元の3ターンはセッション・transcript path・turn numberだけを参照する。
`caveat jev cases --json`と端末内Web UIの`/jev`で確認し、
`caveat jev review <id> correct|incorrect|missed|none|unclear --note "理由"`で正誤を記録する。
集計の分母は人が判定した案件だけとし、解決したかどうかは含めない。
